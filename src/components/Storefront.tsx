...
// sold count) so each card looks populated. These are presentation-only
// placeholders to be swapped for real review data once that exists.
function productSignals(id: number) {
  const seed = (n: number) => {
...
function StarRow({ rating, reviews }: { rating: number; reviews: number }) {
  ...
  return (
    <div className="mt-1.5 flex items-center gap-1.5 text-[11px]" aria-label={`Rated ${rating} of 5 from ${reviews} reviews`}>
...
function ProductCard({
  ...
  return (
    <m.article
      ...
      className={`group relative flex w-full shrink-0 flex-col overflow-hidden rounded-2xl border border-ink/10 bg-white transition-shadow duration-200 hover:border-ink/25 hover:shadow-lg lg:w-48 lg:snap-start ${
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
            {justAdded ? (
              <m.span key="ok" initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }} transition={{ duration: 0.18 }}>
                Added &#10003;
              </m.span>
            ) : (
              <m.span key="add" initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 10, opacity: 0 }} transition={{ duration: 0.18 }}>
                Add to cart
              </m.span>
            )}
          </AnimatePresence>
